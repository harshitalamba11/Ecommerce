...
function scrollLeft(containerId) {
    console.log('scrollLeft called for:', containerId);
    const container = document.getElementById(containerId);
    if (container) {
        container.scrollBy({ left: -300, behavior: 'smooth' });
    } else {
...
const userId = 1; // Ideally this comes from the logged-in user data

const addToCart = async (productId, quantity, price, userId) => {
    try {
...
function handleAddToCart(productId, quantity, price) {
    addToCart(productId, quantity, price, userId);
}
